...
def searchRecipe(arg1, veg=False):
    if veg:
        arg1=+" vegetarien"
...
def confirmRecipe(es, id=0 , recipe="test"):
    ...
    doc=res["_source"]
    if not doc.contains({"url":recipe["url"],"name":recipe["name"],"ingredients":recipe['ingredients'],"step":recipe['steps'],"image":recipe["image"]}):    
