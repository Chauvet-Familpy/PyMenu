...
class Marmiton(object):
    ...
    def get(url):
        ...
        try:
            ...
            ingredients_map={}
            quantity_data=soup.findAll("span", {"class": "recipe-ingredient-qt"})
            ingredients_data=soup.findAll("p", {"class": "name_singular"})["data-name-singular="]
            for i in range(len(ingredients_data)):
                ingredients_map.update({ingredients_data[i].text: quantity_data[i].text})
            ...
            rate = soup.find("span", {"class": "recipe-infos-users__rating"}).text
